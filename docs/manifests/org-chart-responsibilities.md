...
| `darnimos` | DArnimos — double-six dominoes game (project_darnimos) | project_endpoint | Godot 4.6 web export, hotseat 1-4 players + AI seats, Blender pre-rendered tiles with runtime-only pixel filter. `emitter_class: cycle_emitter` — fires `agent_health_check_darnimos` during active foreman cycles. First-fired 2026-08-03 (revival onboarding, operator session); Casey deployment `xxxxxxxx`. No HTTP endpoint. |
