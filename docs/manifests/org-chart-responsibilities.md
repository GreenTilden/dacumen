...
| `gamecast` | gamecast podDAd (project_gamecast_poddad) | project_endpoint | Revenue project — weekly news-tied HTML5 games. Foreman-cadenced with own gamecast-cycle-N nephew lineage. Casey deployment `xxxxxxxx`. First-fired `agent_health_check_gamecast` 2026-05-23 (Della cycle-40 L01); inventoried here Della cycle-41 L01 after operator caught the substrate gap. |
...
| `dacumen` | DAcumen — sanitized public methodology mirror (project_dacumen) | project_endpoint | Source-of-record for THIS manifest (pure bash + jq + git, MIT, public remote `github.com/GreenTilden/dacumen`). Manifest source had been missing from its own manifest until Della cycle-41 L05; YAML self-entry + MD persona-row added atomically. EllaBot first-fire `agent_health_check_dacumen` landed cycle-41 L05. No HTTP endpoint. |
| `governance-thread` | governance-thread — GOV workspace + agent-card-research home (project_governance_thread) | project_endpoint | Cross-BU governance thread workspace (foreman-enabled). Casey deployment `xxxxxxxx`. YAML entry pre-dates the persona table (gov-10, 2026-05-16); MD persona-row added Della cycle-41 L05 alongside `agent_health_check_governance_thread` EllaBot first-fire. No HTTP endpoint. |
