...
| `governance-thread` | governance-thread — GOV workspace + agent-card-research home (project_governance_thread) | project_endpoint | Cross-BU governance thread workspace (foreman-enabled). Casey deployment `09c22323`. YAML entry pre-dates the persona table (gov-10, 2026-05-16); MD persona-row added Della cycle-41 L05 alongside `agent_health_check_governance_thread` EllaBot first-fire. No HTTP endpoint. |
