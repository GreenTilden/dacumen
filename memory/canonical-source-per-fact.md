...
- Decide canonical persistence of GOVERNANCE_RAG_URL: either persist it to the systemd unit (or an EnvironmentFile) so the GOV-09 claim becomes durably true, OR explicitly accept that the hardcoded default is canonical and remove the "deployed env var" claim from sprint-log/memory references. **Still open** — function works via hardcoded default which happens to match `http://192.168.0.151:8002`.