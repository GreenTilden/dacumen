...
- rag-core-client v0.4.0 redeploy across consumers (casey-junior + lorna-financials + ellabot) — same shape as the original RC6 rename. Same memory file applies ([[cascade-rc-rename-consumer-runtime-gap]]). Behavior-changing, prod-touching, needs operator gate.
- Decide canonical persistence of GOVERNANCE_RAG_URL: either persist it to the systemd unit (or an EnvironmentFile) so the GOV-09 claim becomes durably true, OR explicitly accept that the hardcoded default is canonical and remove the "deployed env var" claim from sprint-log/memory references.