...
| `duck` | Chief Ephemeralization Officer | business_role | AI-workload placement (local-GPU-first) + autonomous fleet oversight + agents-graph maintenance. Short-form persona `duck` (23 chars) chosen because full-form `duckminster` = 30 chars, exactly at the EllaBot `source` VARCHAR(30) cap (zero margin). Reports to operator (meta-seat precedent). `emitter_class: cycle_emitter` (fires `agent_health_check_duck`). Minted cycle-82 (professional_82). |
| `operator` | Operator | business_role | Reserved for the future `source: operator_intent` (Layer C contract, Phase 6) |
| `poddad` | gamecast podDAd (project_gamecast_poddad) | project_endpoint | Revenue project — weekly news-tied HTML5 games. Foreman-cadenced with own gamecast-cycle-N nephew lineage. Casey deployment `<deployment-id>`. Canonical EllaBot source is `agent_health_check_poddad` (firing; latest 2026-06-03). `agent_health_check_gamecast` was a one-off 2026-05-24 mis-fire — DEPRECATED per the YAML changelog (cycle-63 L05 contract correction); poddad is canonical for gamecast-poddad. Inventoried here Della cycle-41 L01 after operator caught the substrate gap; source-key corrected `gamecast`→`poddad` Della cycle-63 (matches the YAML entry's `task_sources`). |
...
| `coriolii` | Corîolîî Whether System (project_coriolii) | project_endpoint | Cores-layer governance home — internal upstream to DAcumen. Trio-only (no agent persona). Pillar Professional. Casey deployment `<deployment-id>`. Foreman-cadenced (`coriolii-cycle-N` lineage); trigger-activated but ACTIVE in practice (cycle-23 closed 2026-07-04 — the earlier "steady-state since cycle-7" note was 6 weeks stale, description-decay catch Della cycle-65 L03). Session-fired telemetry lapsed 2026-06-05→07-05; daily heartbeat timer wired 2026-07-05 (`agent-health-check-coriolii.timer`, poddad pattern). YAML + MD persona-row landed Della cycle-41 L03. |
| `dacumen` | DAcumen — sanitized public methodology mirror (project_dacumen) | project_endpoint | Source-of-record for THIS manifest (pure bash + jq + git, MIT, public remote `github.com/GreenTilden/dacumen`). Manifest source had been missing from its own manifest until Della cycle-41 L05; YAML self-entry + MD persona-row added atomically. EllaBot first-fire `agent_health_check_dacumen` landed cycle-41 L05. Casey deployment `<deployment-id>` registered Della cycle-41 L07 atomically (4 needs N-01..N-04 + session_start activity 6b93921a, customer "DArnTech (Internal · Governance)"). No HTTP endpoint. |
| `gov_thread` | governance-thread — GOV workspace + agent-card-research home (project_governance_thread) | project_endpoint | Cross-BU governance thread workspace (foreman-enabled). Casey deployment `<deployment-id>`. YAML entry pre-dates the persona table (gov-10, 2026-05-16); MD persona-row added Della cycle-41 L05; EllaBot first-fire `agent_health_check_gov_thread` landed L06 using **SHORT-FORM persona** `gov_thread` (29 chars) because long-form `governance_thread` (36 chars) exceeds the EllaBot `source` VARCHAR(30) cap. The canonical persona-suffix in this column is **the short-form for source-name purposes**; the long-form persona "governance-thread" remains the human-readable label. See [[feedback_ellabot_source_varchar_30_cap]] for the constraint. No HTTP endpoint. |
| `dq` | DillerQueen — revived arcade game (project_diller_queen) | project_endpoint | Python game-logic server + render-only Godot client; HTML5 primary target, Android APK for ArcadeKit kiosks. `emitter_class: burst_emitter` (v0.2.2) — fires `agent_health_check_dq` only during active build bursts, no daily cadence expected, validator info-only. Registered Della cycle-65 L03 (was live+firing but undeclared). Known gap: dq entries borrow `RND.AI.AUTO` because intended `RND.GAME_DEV` is missing from the EllaBot code registry — operator call, flagged same loop. |
| `forgemaster` | Sprite Forge (project_sprite_forge) | project_endpoint | ComfyUI + Ollama + pgvector RAG sprite-gen pipeline; on-disk consumers lennon-lives / diller-queen / gamecast (coriolii cycle-23 seam detection). `emitter_class: burst_emitter` (v0.2.2) — fires `agent_health_check_forgemaster` during active blitzes, validator info-only. Registered Della cycle-65 L03 (was live+firing but undeclared). |
| `darnimos` | DArnimos — double-six dominoes game (project_darnimos) | project_endpoint | Godot 4.6 web export, hotseat 1-4 players + AI seats, Blender pre-rendered tiles with runtime-only pixel filter. `emitter_class: cycle_emitter` — fires `agent_health_check_darnimos` during active foreman cycles. First-fired 2026-08-03 (revival onboarding, operator session); Casey deployment `<deployment-id>`. No HTTP endpoint. |

**Known unregistered emitters**: none remaining (re-verified Della cycle-65 L03 — validator §2 clean after dq + forgemaster registration). Della cycle-41 L01 surfaced the drift-class (gamecast inventoried L01, proxmox MD-row added L02, coriolii YAML+MD-row added L03). New same-class emitters that surface in future cycles should land here in the same shape; touchpoint #11 in `~/projects/operator-scripts/utility/onboard-project.sh` (`check_dacumen_inventory`) catches the gap going forward.
