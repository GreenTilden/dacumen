...
5. **Grep by the VALUE, not by the env-var/identifier name.** Different consumers often name the same fact differently (`LORNA_TOKEN` vs `LORNA_FINANCIALS_TOKEN` vs `FINANCIALS_API_TOKEN` for the same Lorna API token; `FOO_URL` vs `FOO_BASE_URL` vs `FOO_ENDPOINT` for the same upstream). A name-grep will silently miss any consumer that picked a different convention. The value (a specific token hex, a specific IP literal, a specific version string) is the only identifier guaranteed to be consistent across consumers. **The hour after [[docker-compose-restart-doesnt-reload-env-file]] was codified, this rule was learned the hard way too**: GOV-10 addendum 5 swept by `LORNA_FINANCIALS_TOKEN|FINANCIALS_API_TOKEN` regex, missed `olivers-garage` (which uses `LORNA_TOKEN`), claimed "fully closed," and got proven wrong by the next sweep — which used the token VALUE instead and surfaced the missed consumer on the first hit. Name-grep is a lossy index; value-grep is the canonical-source check.
