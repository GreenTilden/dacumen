...
| `coriolii` | Corîolîî Whether System (project_coriolii) | project_endpoint | Cores-layer governance home — internal upstream to DAcumen. Trio-only (no agent persona). Pillar Professional. Casey deployment `xxxxxxxx`. Foreman-cadenced (`coriolii-cycle-N` lineage); steady-state since cycle-7 close 2026-05-23. YAML + MD persona-row landed Della cycle-41 L03 — gap-class drift fully closed (all three same-class items from L01 finding resolved within cycle-41). |
